...
def process_file(file):
    ...
    saved_model_dir = model_path + '/' + model_name +"/1"
    tag_set = "serve"
    ...
    for signature_def_key in sorted(sig_def.keys()):
        ...
        for input_key, tensor_info in sorted(outputs.items()):

            dtype = {value: key
            for (key, value) in types_pb2.DataType.items()}[tensor_info.dtype]
            dims = [str(dim.size) for dim in tensor_info.tensor_shape.dim]
            name = tensor_info.name
            output_info = {"key": input_key, "dtype": dtype, "dims": dims, "name" : name}
            outputs_info.append(output_info)
        model_info[signature_def_key] = {"inputs": inputs_info, "outputs": output_info}
    print(model_info)
    return model_info
